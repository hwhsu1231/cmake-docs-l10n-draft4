...
if(VERSION_TYPE STREQUAL "branch")
    ...
    message("")
    restore_cmake_message_indent()
    message(STATUS "Getting the latest commit of '${BRANCH_NAME}' from the remote...")
    ...
    execute_process(
        COMMAND
            ${Git_EXECUTABLE} reset --hard FETCH_HEAD
        WORKING_DIRECTORY ${PROJ_OUT_REPO_DIR}
        ECHO_OUTPUT_VARIABLE
        ECHO_ERROR_VARIABLE
        COMMAND_ERROR_IS_FATAL ANY)
...
elseif(VERSION_TYPE STREQUAL "tag")
    message(STATUS "Getting the latest tag of '${TAG_PATTERN}' from the 'origin' remote...")
    ...
    message("")
    restore_cmake_message_indent()
    message(STATUS "Running 'git fetch' command to fetch the latest tag '${LATEST_TAG}' from the 'origin' remote...")
    remove_cmake_message_indent()
    message("")
    execute_process(
        COMMAND
